...
'''
# 字符串处理的时候，最常用的是连接,Python中的字符串与其他的语言有一点不同，
# 它是不可变对象,一旦创建不能改变.
...
# 原因是因为若要连接字符串:S1+S2+S3+....+SN,由于字符串是不可变的对象,执行一次就要申请一块新的内存，这样的话在N个字符串连接的过程中，
# 会产生N-1个中间结果,每产生一个中间结果就要申请一次内存，这样会严重影响执行效率.
...
# 这种else 块会在整个循环执行完之后立刻运行。既然如此，那它为什么叫做else 呢？
# 为什么不叫and ？在if/else 语句中，else 的意思是：如果不执行前面那个if 块，那就执行else 块。
...
# 问题来了对于刚接触Python 的程序员可能会把for/else 结构中的else 块理解为：
# 如果循环没有正常执行完，那就执行else 块。
# 实际上刚好相反——在循环里用break语句提前跳出，会导致程序不执行else 块，这一点会有点绕.
# 对于不熟悉for/else 的人来说,会令阅读代码的人感到相当费解.
